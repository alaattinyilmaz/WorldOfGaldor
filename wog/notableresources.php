...
<center>
  ...
  <table width="1123" height="357" cellpadding="0" cellspacing="0" id="Table_01">
	...
	<tr>
		...
		<div align="center">
	      <p class="style3">NOTABLE RESOURCES</p>
          <ul>
	        <p>The workers  work on the lands to mine, to farm, or to collect the valuable resources. List of resources on the lands that has some workers on it,  with the number of houses that are processing these resources.
	          </li>
                </p>
	        </ul>
            ...
            <thead>
              <tr>
          <th width="264" bgcolor="#333333" class="style8">Notable Resource </th>
          <th width="129" bgcolor="#333333" class="style8">Popularity</th>
      </tr>
            </thead>
            <tbody>
...
$sql = 'SELECT L.notableResources, COUNT(L.notableResources) as FARM
FROM worker W, land L
WHERE W.farms = L.lID
GROUP BY L.notableResources
ORDER BY FARM DESC';
...
$sqlCount = 'SELECT COUNT(*) as count FROM land';
...
?>
  ...
  <?php
    ...
    $resource = $row['notableResources'];
    $popularity = $row['FARM'];
	
    echo "<tr>
          <td><center><font color='#e88c42'>$resource</font></center></td>
          <td><center><font color='#9fe6e5'>$popularity</font></center></td>
          </tr>";
}
